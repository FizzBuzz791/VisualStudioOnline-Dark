		...
		(
			...
			FROM dbo.Stockpile AS sp
				LEFT OUTER JOIN dbo.BhpbioStockpileLocationDate AS spl
					...
					AND ([Start_Date] BETWEEN @MonthStartDate AND @MonthEndDate
						OR End_Date BETWEEN @MonthStartDate AND @MonthEndDate
						OR ([Start_Date] < @MonthStartDate AND End_Date >@MonthEndDate))
			...
			(				
				@iLocationId IS NULL
					...
					(	
						...
						FROM dbo.GetBhpbioReportLocationBreakdownWithOverride(@iLocationId,0,NULL,@MonthStartDate,@MonthEndDate)
					)
			)
