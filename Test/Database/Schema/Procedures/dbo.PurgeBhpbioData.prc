...
DECLARE 
	...
	DELETE g
	FROM dbo.BhpbioShippingNominationItemParcelGrade g 
		INNER JOIN dbo.BhpbioShippingNominationItemParcel p WITH (NOLOCK)
			ON p.BhpbioShippingNominationItemParcelId = g.BhpbioShippingNominationItemParcelId
		INNER JOIN dbo.BhpbioShippingNominationItem n WITH (NOLOCK)
			ON p.BhpbioShippingNominationItemId = n.BhpbioShippingNominationItemId
	WHERE CONVERT(VARCHAR(6),n.OfficialFinishTime,112) <= @Compare
	
	DELETE p
	FROM dbo.BhpbioShippingNominationItemParcel p
		INNER JOIN dbo.BhpbioShippingNominationItem n WITH (NOLOCK)
			ON p.BhpbioShippingNominationItemId = n.BhpbioShippingNominationItemId
	WHERE CONVERT(VARCHAR(6),n.OfficialFinishTime,112) <= @Compare

	DELETE dbo.BhpbioShippingNominationItem
