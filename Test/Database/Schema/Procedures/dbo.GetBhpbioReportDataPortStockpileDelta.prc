	...
	BEGIN TRY
		...
		BEGIN
			...
			INSERT INTO @PortDelta
				(CalendarDate, DateFrom, DateTo, ParentLocationId, LastBalanceDate, ProductSize, Tonnes, LastTonnes)
			SELECT B.CalendarDate, B.DateFrom, B.DateTo, L.ParentLocationId, BPBPREV.BalanceDate, 
				CASE WHEN BPB.ProductSize = 'ROM' THEN defaultlf.ProductSize ELSE BPB.ProductSize END,
				Sum(ISNULL(defaultlf.[Percent], 1) * BPB.Tonnes), 
				Sum(ISNULL(defaultlfprev.[Percent], 1) * BPBPREV.Tonnes)
			FROM dbo.GetBhpbioReportBreakdown(@iDateBreakdown, @iDateFrom, @iDateTo, 1 /* do not include data before start date */) AS B
				INNER JOIN @PortBalance AS BPB
			...
			GROUP BY B.CalendarDate, B.DateFrom, B.DateTo, BPBPREV.BalanceDate, L.ParentLocationId, CASE WHEN BPB.ProductSize = 'ROM' THEN defaultlf.ProductSize ELSE BPB.ProductSize END
			
			-- roll up lump/fines data for total
			INSERT INTO @PortDelta
				(CalendarDate, DateFrom, DateTo, ParentLocationId, LastBalanceDate, ProductSize, Tonnes, LastTonnes)
			SELECT CalendarDate, DateFrom, DateTo, ParentLocationId, LastBalanceDate, 'TOTAL', SUM(Tonnes), SUM(LastTonnes)
			FROM @PortDelta
			GROUP BY CalendarDate, DateFrom, DateTo, ParentLocationId, LastBalanceDate
		END
		...
		BEGIN
			...
			INSERT INTO @PortDelta
				(CalendarDate, DateFrom, DateTo, ParentLocationId, LastBalanceDate, ProductSize, Tonnes, LastTonnes)
			SELECT s.CalendarDate, s.DateFrom, s.DateTo, l.ParentLocationId, NULL, s.ProductSize, s.Tonnes, 0
		...
		SELECT CalendarDate, DateFrom, DateTo, NULL As MaterialTypeId, ParentLocationId,
			Coalesce(Tonnes - LastTonnes, 0) AS Tonnes, ProductSize
		FROM @PortDelta		
