	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				...
				-- 1. The Site's location recorded against the Crusher  (this is the permanent and not dynamic location assignment)..
			...
			BEGIN
				...
				-- : The Code directly mapped to the Stockpile's Name
				
				SET @StockpileId = 
					(
						SELECT s.Stockpile_Id
						FROM dbo.Stockpile AS s
						WHERE s.Stockpile_Name = @iCode
					)
			END
			...
			BEGIN
				...
				BEGIN
					...
					-- Note: The selection made here may be revised in the production import post-process step based on other considerations (dynamic location hierarchy)
