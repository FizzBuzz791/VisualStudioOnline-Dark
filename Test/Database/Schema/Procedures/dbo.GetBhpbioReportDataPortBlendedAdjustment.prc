...
BEGIN 
	...
	(
		...
		Tonnes FLOAT,
		Removal BIT,
		GeometMovementType VARCHAR(32) DEFAULT 'NI'
	)
	...
	BEGIN TRY
		...
		BEGIN
			INSERT INTO @Blending
				(CalendarDate, DateFrom, DateTo, BhpbioPortBlendingId, ParentLocationId, ProductSize, Tonnes, Removal, GeometMovementType)
			...
			FROM dbo.GetBhpbioReportBreakdown(@iDateBreakdown, @iDateFrom, @iDateTo, 1 /* do not include data before start date */) AS B
				INNER JOIN dbo.GetBhpbioPortBlendingForGeomet(@iDateFrom, @iDateTo) AS BPB -- BhpbioPortBlending as BPB
			...
			WHERE	-- where there is no associated approval OR there is and Approved data is not being included
				(bad.TagId IS NULL OR @iIncludeApprovedData = 0)
				AND	HXF.LocationId IS NULL
				AND	ISNULL(defaultlf.[Percent], 1) > 0
			
			-- roll up lump/fines tonnes for total
			--
			-- Note that the GMT for TOTALS are *always* NI
			INSERT INTO @Blending
				(CalendarDate, DateFrom, DateTo, BhpbioPortBlendingId, ParentLocationId, ProductSize, Tonnes, Removal, GeometMovementType)
			SELECT CalendarDate, DateFrom, DateTo, BhpbioPortBlendingId, ParentLocationId, 'TOTAL', SUM(Tonnes), Removal, 'NI'
			FROM @Blending
			GROUP BY CalendarDate, DateFrom, DateTo, BhpbioPortBlendingId, ParentLocationId, Removal
			...
			SELECT B.CalendarDate,  B.DateFrom, B.DateTo, B.ParentLocationId,
				BPBG.GradeId,
				SUM(ABS(B.Tonnes) * BPBG.GradeValue) / NULLIF(SUM(ABS(B.Tonnes)), 0) AS GradeValue, B.ProductSize,
				SUM(ABS(B.Tonnes)),
				B.GeometMovementType
			FROM @Blending AS B
				INNER JOIN dbo.BhpbioPortBlendingGrade AS BPBG
					ON BPBG.BhpbioPortBlendingId = B.BhpbioPortBlendingId
			GROUP BY B.CalendarDate, B.DateFrom, B.DateTo, B.ParentLocationId, BPBG.GradeId, B.ProductSize, B.GeometMovementType
		END
		...
		BEGIN
			...
			-- Retrieve Tonnes
			INSERT INTO @Blending
				(CalendarDate, DateFrom, DateTo, ParentLocationId, ProductSize, Tonnes, Removal)
			SELECT s.CalendarDate, s.DateFrom, s.DateTo, l.ParentLocationId, s.ProductSize, ABS(s.Tonnes), CASE WHEN s.Tonnes < 0 THEN 1 ELSE 0 END
			...
			SELECT s.CalendarDate, s.DateFrom, s.DateTo,  l.ParentLocationId,
					s.GradeId, s.GradeValue, s.ProductSize, s.Tonnes
		...
		SELECT B.CalendarDate, B.DateFrom, B.DateTo, B.ParentLocationId, NULL AS MaterialTypeId, ProductSize,
			Sum(CASE WHEN B.Removal = 0 THEN B.Tonnes ELSE -B.Tonnes END) AS Tonnes
		FROM @Blending AS B
		WHERE B.GeometMovementType <> 'I'
		GROUP BY B.CalendarDate, B.DateFrom, B.DateTo, B.ParentLocationId, B.ProductSize
		
		
		SELECT BG.CalendarDate, G.Grade_Name AS GradeName, BG.ParentLocationId,
			NULL AS MaterialTypeId, ProductSize,
		...
		WHERE BG.GeometMovementType <> 'I'
		GROUP BY BG.CalendarDate, BG.ParentLocationId, G.Grade_Name, BG.ProductSize
