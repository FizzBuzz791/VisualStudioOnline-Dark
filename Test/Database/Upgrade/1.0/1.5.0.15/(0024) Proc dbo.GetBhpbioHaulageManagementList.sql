	...
	BEGIN TRY
		...
		From dbo.Haulage As h
			...
			Left Join dbo.BhpbioStockpileLocationDate as sl
				...
				AND	(sl.[Start_Date] BETWEEN @iFilter_Start_Date AND @iFilter_End_Date
					OR sl.End_Date BETWEEN @iFilter_Start_Date AND @iFilter_End_Date
					OR (sl.[Start_Date] < @iFilter_Start_Date AND sl.End_Date >@iFilter_End_Date))
			...
			Left Join dbo.BhpbioStockpileLocationDate As dsl
				On (h.Destination_Stockpile_Id = dsl.Stockpile_Id)
				AND	(dsl.[Start_Date] BETWEEN @iFilter_Start_Date AND @iFilter_End_Date
					OR dsl.End_Date BETWEEN @iFilter_Start_Date AND @iFilter_End_Date
					OR (dsl.[Start_Date] < @iFilter_Start_Date AND dsl.End_Date >@iFilter_End_Date))
		...
		Where (Coalesce(H.Source_Digblock_Id, Cast(H.Source_Stockpile_Id As Varchar), H.Source_Mill_Id) = @iFilter_Source Or @iFilter_Source Is Null)
			...
			AND EXISTS
				(
					...
					FROM dbo.GetBhpbioReportLocationBreakdownWithOverride(@iLocation_Id,0,NULL,@iFilter_Start_Date,@iFilter_End_Date) AS l2
					WHERE l2.LocationId = dl.Location_Id
						OR l2.LocationId = sl.Location_Id
						OR l2.LocationId = ml.Location_Id
						OR l2.LocationId = cl.Location_Id
						OR l2.LocationId = dsl.Location_Id
						OR l2.LocationId = dml.Location_Id
				)
...
EXEC dbo.GetBhpbioHaulageManagementList
	@iFilter_Start_Date = '01-Jul-2012',
	@iFilter_Start_Shift = 'D',
	@iFilter_End_Date = '31-Jul-2012',
	@iFilter_End_Shift = 'D',
	@iFilter_Source = NULL,
