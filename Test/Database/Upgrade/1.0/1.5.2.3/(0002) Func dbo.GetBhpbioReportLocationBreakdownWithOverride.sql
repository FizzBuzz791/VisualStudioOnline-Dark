...
BEGIN
	...
	-- If "GetChildLocations" = 1, only the children and downwards of the specified location will be returned 
	-- (i.e. excludes the location which corresponds to @iLocationId), and the column ParentLocationId 
	-- will be populated with locationid at the top of the remaining location hierarchy.  For example, if a
	-- Hub location is given, all of the ParentLocationId's will correspond to Site locations
	
	-- If "GetChildLocations" = 0, then the full hierarchy is returned (including @iLocationId) and the 
	-- returned ParentLocationId's are NOT resolved
	...
	BEGIN
		INSERT INTO @ParentLocation
			(LocationId)
		SELECT Location_Id
		FROM dbo.BhpbioLocationDate AS L
		WHERE L.Parent_Location_Id = @LocationId
	END
	
	-- get initial seed location records
	...
	AND	(L.[Start_Date] BETWEEN @iDateFrom AND @iDateTo
				OR L.End_Date BETWEEN @iDateFrom AND @iDateTo
				OR (L.[Start_Date] < @iDateFrom AND L.End_Date > @iDateTo))	
	---------------------------------------------
	...
	AND ((NOT L.Location_Type_Id = 3) OR (L.[Start_Date] <= @iDateFrom))
	---------------------------------------------
