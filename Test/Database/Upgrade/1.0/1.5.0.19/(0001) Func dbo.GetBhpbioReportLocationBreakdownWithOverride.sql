...
BEGIN
	...
	-- The date of Jimblebar site move
	DECLARE @JimblebarSiteMoveCutOff DATETIME
	SET @JimblebarSiteMoveCutOff = '2012-10-01'
	...
	(     
		 SELECT	L.Location_Id, L.Location_Type_Id, L.Location_Id, L.[Start_Date], L.End_Date, L.Parent_Location_Id
		  ...
		  FROM		BhpbioLocationDate L
			LEFT JOIN BhpbioLocationDate ParentHub ON ParentHub.Location_Id = L.Parent_Location_Id AND ParentHub.Location_type_Id = 2
		  WHERE		L.Location_Id = @LocationId
			---------------------------------------------
			-- Special clause here to exlcude Sites moved from one hub to another where the @iDateFrom is prior to the start date of the location
			-- This is here to cater for the Jimblebar site requirement, where reporting against Jimblebar site should NOT be done across cutover boundaries
			-- This could be more generically written as @iDateFrom < L.[Start_Date] however this may have unwanted effects for new sites added in the future
			AND (ParentHub.Location_Id IS NULL OR NOT (@iDateFrom < @JimblebarSiteMoveCutOff AND L.[Start_Date]>=@JimblebarSiteMoveCutOff))
			---------------------------------------------
			---------------------------------------------
		    AND		L.Location_Type_Id <= @LowestLevel + 1
		  ...
		  WHERE		L.Location_Type_Id <= @LowestLevel + 1
