...
BEGIN 
	...
	BEGIN TRY
		...
		FROM dbo.GetBhpbioReportLocationBreakdownWithOverride(@iLocationId,0,NULL,@iDateFrom,@iDateTo)
		...
		FROM (
			...
			WHERE RealTonnes <> 0
			UNION ALL
			SELECT DISTINCT W.WeightometerSampleId, w.DesignationMaterialTypeId,
		...
		BEGIN
			SELECT DISTINCT H.Haulage_Id, H.Haulage_Date, H.Tonnes, S.Stockpile_Name As Destination_Stockpile, 
				Fe.Grade_Value As Fe, P.Grade_Value As P, SiO2.Grade_Value As SiO2, Al2O3.Grade_Value As Al2O3, LOI.Grade_Value As LOI,
		...
		BEGIN
			SELECT DISTINCT H.Haulage_Id, H.Haulage_Date, H.Tonnes, 
				Fe.Grade_Value As Fe, P.Grade_Value As P, SiO2.Grade_Value As SiO2, Al2O3.Grade_Value As Al2O3, LOI.Grade_Value As LOI,
		...
		BEGIN
			...
			From dbo.BhpbioShippingTransactionNomination AS S
				INNER JOIN dbo.Location AS L
					...
					--AND S.OfficialFinishTime BETWEEN L.Start_Date AND L.End_Date)
				INNER JOIN @Location AS FL
					ON (L.Location_Id = FL.LocationId
					AND S.OfficialFinishTime BETWEEN FL.IncludeStart AND FL.IncludeEnd)
		...
		BEGIN
			...
			SELECT DISTINCT S.Stockpile_Id, WS.Weightometer_Sample_Id, 0, WS.Tonnes, LL.Description
			FROM dbo.WeightometerSample AS WS
				INNER JOIN dbo.Stockpile AS S
			...
			INSERT INTO @StockpileDeltaHub
				(StockpileId, WeightometerSampleId, Addition, Tonnes,Hub)		
			SELECT DISTINCT S.Stockpile_Id, WS.Weightometer_Sample_ID, 1, WS.Tonnes, LL.Description
			FROM dbo.WeightometerSample AS WS
				INNER JOIN dbo.Stockpile AS S
					ON (S.Stockpile_Id = WS.Destination_Stockpile_Id)
				INNER JOIN dbo.StockpileGroupStockpile AS SGS
			...
			SELECT DISTINCT SD.WeightometerSampleId, WS.Weightometer_Sample_Date, CASE WHEN SD.Addition = 1 THEN SD.Tonnes ELSE -SD.Tonnes END AS MovementTonnes, 
			...
			FROM @StockpileDeltaHub AS SD
				...
				WHERE xs.StockpileId IS NULL -- No movements to or from excluded groups.
			UNION ALL
			SELECT DISTINCT ws.Weightometer_Sample_Id, WS.Weightometer_Sample_Date, NULL AS MovementTonnes, WSV.Field_Value As SampleTonnes,
				Fe.Grade_Value As Fe, P.Grade_Value As P, SiO2.Grade_Value As SiO2, Al2O3.Grade_Value As Al2O3, LOI.Grade_Value As LOI,
		...
		BEGIN
			...
			FROM dbo.BhpbioPortBlending AS BPB
				...
				INNER JOIN dbo.Location AS RH
					...
					AND RH.Location_Id NOT IN (SELECT LocationId FROM GetBhpbioExcludeHubLocation('PortBlending'))
