...
#Region "Private Methods"
    ...
    Private Shared Function GetHubReportSet(session As ReportSession,
        ...
        Dim breakdown = ReportBreakdown.Monthly

        session.CalculationParameters(dateFrom, dateTo, breakdown, locationId, Nothing)
        ...
        Dim geologyModel As CalculationResult = Calculation.Create(CalcType.ModelGeology, session).Calculate()
        ...
        Dim shortTermGeologyModel As CalculationResult = Calculation.Create(CalcType.ModelShortTermGeology, session).Calculate()
        ...
        If Not f3Calc Is Nothing Then
            ...
            If session.OptionalCalculationTypesToInclude.Contains(CalcType.ModelMiningBene) Then
                ...
                If Not hasBene Then miningModelBeneAdjusted = Nothing
            End If
        End If

        'Determine if we should be showing it.
        Dim valid As Boolean = FactorLocation.IsLocationInLocationType(session, session.RequestParameter.LocationId, "HUB")

        ' add the validation flags to the results (where we have them)
        If Not miningModelCrusherEquivalentResult Is Nothing Then miningModelCrusherEquivalentResult.Tags.Add(New CalculationResultTag("PresentationValid", GetType(Boolean), valid))
        If Not postCrusherStockpileDeltaResult Is Nothing Then postCrusherStockpileDeltaResult.Tags.Add(New CalculationResultTag("PresentationValid", GetType(Boolean), valid))
        If Not sitePostCrusherStockpileDeltaResult Is Nothing Then sitePostCrusherStockpileDeltaResult.Tags.Add(New CalculationResultTag("PresentationValid", GetType(Boolean), valid))
        If Not hubPostCrusherStockpileDeltaResult Is Nothing Then hubPostCrusherStockpileDeltaResult.Tags.Add(New CalculationResultTag("PresentationValid", GetType(Boolean), valid))
        If Not portBlendedAdjustmentResult Is Nothing Then portBlendedAdjustmentResult.Tags.Add(New CalculationResultTag("PresentationValid", GetType(Boolean), valid))
        If Not portStockpileDeltaResult Is Nothing Then portStockpileDeltaResult.Tags.Add(New CalculationResultTag("PresentationValid", GetType(Boolean), valid))
        ...
        If Not geologyModel Is Nothing Then holdingData.Add(geologyModel)
        If Not geologyModelH2OAsDropped Is Nothing Then holdingData.Add(geologyModelH2OAsDropped)
        If Not geologyModelH2OAsShipped Is Nothing Then holdingData.Add(geologyModelH2OAsShipped)

        If Not f15Calc Is Nothing Then holdingData.Add(f15Calc.GetFirstCalcId(ModelShortTermGeology.CalculationId))
        If Not f1Calc Is Nothing Then holdingData.Add(f1Calc.GetFirstCalcId(ModelMining.CalculationId))

        If Not miningModelBeneAdjusted Is Nothing Then holdingData.Add(miningModelBeneAdjusted)
        If Not miningModelH2OAsDropped Is Nothing Then holdingData.Add(miningModelH2OAsDropped)
        If Not miningModelH2OAsShipped Is Nothing Then holdingData.Add(miningModelH2OAsShipped)

        If Not f1Calc Is Nothing Then holdingData.Add(f1Calc.GetFirstCalcId(ModelGradeControl.CalculationId))
        If Not f15Calc Is Nothing Then holdingData.Add(f15Calc.GetFirstCalcId(ModelGradeControlSTGM.CalculationId))
        ...
        If Not miningModelCrusherEquivalentResult Is Nothing Then holdingData.Add(miningModelCrusherEquivalentResult)
        If Not sitePostCrusherStockpileDeltaResult Is Nothing Then holdingData.Add(sitePostCrusherStockpileDeltaResult)
        If Not hubPostCrusherStockpileDeltaResult Is Nothing Then holdingData.Add(hubPostCrusherStockpileDeltaResult)
        If Not postCrusherStockpileDeltaResult Is Nothing Then holdingData.Add(postCrusherStockpileDeltaResult)
        If Not f25Calc Is Nothing Then holdingData.Add(f25Calc.GetFirstCalcId(OreForRail.CalculationId))
        If Not f25Calc Is Nothing Then holdingData.Add(f25Calc.GetFirstCalcId(MiningModelOreForRailEquivalent.CalculationId))
        ...
        If Not f2Calc Is Nothing Then
            ...
            If Not f2DensityCalc Is Nothing Then
                ...
                Dim actualMined As CalculationResult = f2DensityCalc.GetFirstCalcId(Calc.ActualMined.CalculationId)
                If Not actualMined Is Nothing Then holdingData.Add(actualMined)
        ...
        If f15Calc Is Nothing Then
            ...
            ' doubled up
            If Not shortTermGeologyModel Is Nothing Then holdingData.Add(shortTermGeologyModel)
        End If

        If (session.OptionalCalculationTypesToInclude.Count > 0) Then
            For Each calcTypeToInclude As CalcType In session.OptionalCalculationTypesToInclude
                If calcTypeToInclude = CalcType.ModelMiningBene Or
                   ...
                   calcTypeToInclude = CalcType.F05 Then
                    Continue For
                End If
                Dim result As CalculationResult = Calculation.Create(calcTypeToInclude, session).Calculate()
                ...
                If Not result Is Nothing Then
                    ...
                    If result.CalculationType = CalculationResultType.Ratio Then
                        For Each calcResult In result.GetAllCalculations()
                            If calcResult.CalcId = result.CalcId Then Continue For
        ...
        '        If Not oreForeRailResult Is Nothing Then holdingData.Add(oreForeRailResult)
        If Not portStockpileDeltaResult Is Nothing Then holdingData.Add(portStockpileDeltaResult)
        If Not portBlendedAdjustmentResult Is Nothing Then holdingData.Add(portBlendedAdjustmentResult)
        '        If Not miningModelOreForRailEquivalentResult Is Nothing Then holdingData.Add(miningModelOreForRailEquivalentResult)

        If Not f3Calc Is Nothing Then holdingData.Add(f3Calc.GetFirstCalcId(MiningModelShippingEquivalent.CalculationId))
        ...
        If Not f3Calc Is Nothing Then holdingData.Add(f3Calc.GetFirstCalcId(PortOreShipped.CalculationId))

        If Not f0Result Is Nothing Then holdingData.Add(f0Result)
        If Not f05Result Is Nothing Then holdingData.Add(f05Result)
        If Not f1Calc Is Nothing Then holdingData.Add(f1Calc)
        If Not f15Calc Is Nothing Then holdingData.Add(f15Calc)

        If Not rfgmResult Is Nothing Then holdingData.Add(rfgmResult)
        If Not rfmmResult Is Nothing Then holdingData.Add(rfmmResult)
        If Not rfstmResult Is Nothing Then holdingData.Add(rfstmResult)

        If Not f2Calc Is Nothing Then holdingData.Add(f2Calc)
        If Not f25Calc Is Nothing Then holdingData.Add(f25Calc)
        If Not f3Calc Is Nothing Then holdingData.Add(f3Calc)
    ...
    Private Shared Sub MakeTheZeroRecordsNull(result As DataTable)
        ...
        For Each row As DataRow In result.Rows
            For Each col As String In attributeNames
    ...
    Public Shared Function UnpivotDataTable(table As DataTable, Optional maintainTonnes As Boolean = False) As DataTable
        ...
        For Each attributeName In attributeNames
            If maintainTonnes And attributeName = "Tonnes" Then
                Continue For
            End If
            If table.Columns.Contains(attributeName) Then
    ...
    Public Shared Function Aggregate(resultsToAggregate As DataTable,
                                      referenceTime As DateTime,
        ...
        For Each row As DataRow In resultsToAggregate.Rows()
            ...
            If retainCalendarGroupings OrElse calendarDateHashSet.Count = 0 Then
                If Not calendarDateHashSet.Contains(DirectCast(row("CalendarDate"), DateTime)) Then
        ...
        For Each tag As String In tags
            For Each calendarDate As DateTime In calendarDateList
                firstRow = True
                resultRow = Nothing
                ...
                If retainCalendarGroupings Then
                    filterExpression = String.Format("TagId = '{0}' AND CalendarDate = '{1:yyyy-MM-dd}'", tag, calendarDate)
                Else
                    filterExpression = String.Format("TagId = '{0}'", tag)
                End If

                For Each currentRow As DataRow In resultsToAggregate.Select(filterExpression)
                    ...
                    If firstRow OrElse (Not currentRow("DodgyAggregateGradeTonnes") Is DBNull.Value) Then
                        'sum the tonnes
                        For Each tonnesName As String In New String() {"Tonnes", "Volume", "TonnesDifference"}
                            If firstRow Then
                     ...
                     AndAlso Not (currentRow("DodgyAggregateGradeTonnes") Is DBNull.Value) Then
                        ...
                        For Each gradeName As String In attributeNames
                ...
                If Not resultRow Is Nothing Then
                    If Not ratio Then
                        'only mass weight the grades
                        For Each target As String In attributeNames
                            If Not resultRow(target) Is DBNull.Value Then
                                resultRow(target) = DirectCast(resultRow(target), Double) / DirectCast(resultRow("DodgyAggregateGradeTonnes"), Double)
                            End If
                        Next
                    End If
                End If
    ...
    Public Shared Function CalculateF1F2F3Factors(resultTable As DataTable, Optional ByVal groupOnCalendarDate As Boolean = False, Optional ByVal includeFactorPrefixOnCalcIds As Boolean = True, Optional ByVal locationId As Integer? = Nothing) As DataTable
        Dim adSuffix = "ADForTonnes"
        ...
        If (Not resultTable Is Nothing) Then
            ...
            For Each row As DataRow In matchedRows
                ...
                If (Not String.IsNullOrEmpty(resourceClassification)) Then
                    rowKey = String.Format("{0}_{1}", rowKey, resourceClassification.ToUpper)
                End If

                If (groupOnCalendarDate) Then
                    calendarDates.Add(CDate(row.Item("CalendarDate")))
                    rowKey = rowKey + "_" + String.Format("{0:yyyyMMdd}", row.Item("CalendarDate"))
                End If
        ...
        For Each calendarDate In calendarDates
            ...
            Dim calendarDateRowKeySuffix As String = String.Empty
            If groupOnCalendarDate Then
                calendarDateRowKeySuffix = "_" + String.Format("{0:yyyyMMdd}", calendarDate)
            End If

            ' Iterate for each ProductSize in the set
            For Each productSize As String In productSizes
                For Each resourceClassification As String In resourceClassifications

                    Dim resourceClassificationTagSuffix As String = String.Empty
                    ...
                    If String.IsNullOrEmpty(resourceClassification) Then
                        ...
                        If productSize = "GEOMET" Then
                            rowIndexByTagDictionary.TryGetValue(f3Prefix & MiningModelShippingEquivalent.CalculationId & adSuffix & productSize & calendarDateRowKeySuffix, dataRowF3MiningModelShippingEquivalent)
    ...
    '
    Public Shared Function RecalculateF1F2F3FactorsForUnpivotedTable(resultTable As DataTable, Optional ByVal useCalculationPrefixes As Boolean = True) As DataTable

        Dim recalculateDifference = True
        ...
        Dim factorList As Dictionary(Of String, String()) = GetFactorComponentList(useCalculationPrefixes)
    ...
    <Obsolete("This method is deprecated, use RecalculateF1F2F3FactorsForUnpivotedTable instead")>
    Public Shared Function RecalculateF1F2F3FactorsByCalculationIdLookup(resultTable As DataTable) As DataTable
        ' Instantiate a set of dictionaries that will hold the factor rows themselves along with the source rows
        ' by a key based on calendar date and attribute
        Dim f1RowDictionary As New Dictionary(Of String, DataRow)
        Dim f1GradeControlRowDictionary As New Dictionary(Of String, DataRow)
        Dim f1MiningModelRowDictionary As New Dictionary(Of String, DataRow)

        Dim f15RowDictionary As New Dictionary(Of String, DataRow)
        Dim f15GradeControlSTGMRowDictionary As New Dictionary(Of String, DataRow)
        Dim f15STGMRowDictionary As New Dictionary(Of String, DataRow)

        Dim f2RowDictionary As New Dictionary(Of String, DataRow)
        Dim f2MineProductionExpitEquivalentRowDictionary As New Dictionary(Of String, DataRow)

        Dim f25RowDictionary As New Dictionary(Of String, DataRow)
        Dim f25OreForRailRowDictionary As New Dictionary(Of String, DataRow)
        Dim f25MiningModelOreForRailEquivalentRowDictionary As New Dictionary(Of String, DataRow)

        Dim f3RowDictionary As New Dictionary(Of String, DataRow)
        Dim f3OreShippedRowDictionary As New Dictionary(Of String, DataRow)
        Dim f3MiningModelShippingEquivalentRowDictionary As New Dictionary(Of String, DataRow)


        Dim rfgmRowDictionary As New Dictionary(Of String, DataRow)
        Dim rfGeologyModelRowDictionary As New Dictionary(Of String, DataRow)
        Dim rfmmRowDictionary As New Dictionary(Of String, DataRow)
        Dim rfMiningModelRowDictionary As New Dictionary(Of String, DataRow)
        Dim rfstmRowDictionary As New Dictionary(Of String, DataRow)
        Dim rfShortTermModelRowDictionary As New Dictionary(Of String, DataRow)
        Dim rfMineProductionExpitEquivalentRowDictionary As New Dictionary(Of String, DataRow)



        ' Iterate through each row and entries into the appropriate dictionaries
        For Each resultRow As DataRow In resultTable.Rows
            Dim rowCalcId = CStr(resultRow("CalcId"))
            Dim rowDateTime = CDate(resultRow("CalendarDate"))
            Dim attribute = CStr(resultRow("Attribute"))
            Dim location As String = String.Empty
            Dim productSize As String = String.Empty

            If (Not (resultRow("LocationId") Is Nothing OrElse resultRow("LocationId") Is DBNull.Value)) Then
                location = CStr(resultRow("LocationId"))
            End If

            If (Not (resultRow("ProductSize") Is Nothing OrElse resultRow("ProductSize") Is DBNull.Value)) Then
                productSize = CStr(resultRow("ProductSize"))
            End If

            ' Build a key for storage / lookup of the dictionaries used to store values during factor recalculation
            Dim dictionaryKey As String = String.Format("|{0}|_|{1}|_|{2}|_|{3}|", rowDateTime, attribute, location, productSize)

            ' Note: CalcId is used here rather than TagId... so the LUMP and FINES differntiator does not need to be catered for explicitly
            Select Case rowCalcId
                Case F1.CalculationId
                    f1RowDictionary.Add(dictionaryKey, resultRow)
                Case ModelGradeControl.CalculationId
                    f1GradeControlRowDictionary.Add(dictionaryKey, resultRow)
                Case ModelMining.CalculationId
                    f1MiningModelRowDictionary.Add(dictionaryKey, resultRow)
                    rfMiningModelRowDictionary.Add(dictionaryKey, resultRow)
                Case F15.CalculationId
                    f15RowDictionary.Add(dictionaryKey, resultRow)
                Case ModelGeology.CalculationId
                    rfGeologyModelRowDictionary.Add(dictionaryKey, resultRow)
                Case ModelGradeControlSTGM.CalculationId
                    f15GradeControlSTGMRowDictionary.Add(dictionaryKey, resultRow)
                Case ModelShortTermGeology.CalculationId
                    f15STGMRowDictionary.Add(dictionaryKey, resultRow)
                    rfShortTermModelRowDictionary.Add(dictionaryKey, resultRow)
                Case F2.CalculationId
                    f2RowDictionary.Add(dictionaryKey, resultRow)
                Case MineProductionExpitEquivalent.CalculationId
                    f2MineProductionExpitEquivalentRowDictionary.Add(dictionaryKey, resultRow)
                    rfMineProductionExpitEquivalentRowDictionary.Add(dictionaryKey, resultRow)
                Case F25.CalculationId
                    f25RowDictionary.Add(dictionaryKey, resultRow)
                Case OreForRail.CalculationId
                    f25OreForRailRowDictionary.Add(dictionaryKey, resultRow)
                Case MiningModelOreForRailEquivalent.CalculationId
                    f25MiningModelOreForRailEquivalentRowDictionary.Add(dictionaryKey, resultRow)

                Case PortOreShipped.CalculationId
                    f3OreShippedRowDictionary.Add(dictionaryKey, resultRow)
                Case MiningModelShippingEquivalent.CalculationId
                    f3MiningModelShippingEquivalentRowDictionary.Add(dictionaryKey, resultRow)
                Case F3.CalculationId
                    f3RowDictionary.Add(dictionaryKey, resultRow)

                Case RFGM.CalculationId
                    rfgmRowDictionary.Add(dictionaryKey, resultRow)
                Case RFMM.CalculationId
                    rfmmRowDictionary.Add(dictionaryKey, resultRow)
                Case RFSTM.CalculationId
                    rfstmRowDictionary.Add(dictionaryKey, resultRow)

            End Select
        Next

        ' recalculate F1 factor values
        RecalculateFactorValueBasedOnDataRowDictionaries(f1RowDictionary, f1GradeControlRowDictionary, f1MiningModelRowDictionary)
        ' recalculate F15 factor values
        RecalculateFactorValueBasedOnDataRowDictionaries(f15RowDictionary, f15GradeControlSTGMRowDictionary, f15STGMRowDictionary)
        ' recalculate F2 factor values
        RecalculateFactorValueBasedOnDataRowDictionaries(f2RowDictionary, f2MineProductionExpitEquivalentRowDictionary, f1GradeControlRowDictionary)
        ' recalculate F25 factor values
        RecalculateFactorValueBasedOnDataRowDictionaries(f25RowDictionary, f25OreForRailRowDictionary, f25MiningModelOreForRailEquivalentRowDictionary)
        ' recalculate F3 factor values
        RecalculateFactorValueBasedOnDataRowDictionaries(f3RowDictionary, f3OreShippedRowDictionary, f3MiningModelShippingEquivalentRowDictionary)
        'Recalculate RFGM
        RecalculateFactorValueBasedOnDataRowDictionaries(rfgmRowDictionary, rfMineProductionExpitEquivalentRowDictionary, rfGeologyModelRowDictionary)
        'Recalculate RFMM
        RecalculateFactorValueBasedOnDataRowDictionaries(rfmmRowDictionary, rfMineProductionExpitEquivalentRowDictionary, rfMiningModelRowDictionary)
        'Recalculate RFSTM
        RecalculateFactorValueBasedOnDataRowDictionaries(rfstmRowDictionary, rfMineProductionExpitEquivalentRowDictionary, rfShortTermModelRowDictionary)
    ...
    Private Shared Sub RecalculateFactorValueBasedOnDataRowDictionaries(factorRowDictionary As Dictionary(Of String, DataRow), numeratorSourceRowDictionary As Dictionary(Of String, DataRow), denominatorSourceRowDictionary As Dictionary(Of String, DataRow))
        Dim attributeValueColumnName = "AttributeValue"

        For Each factorDicationaryEntry In factorRowDictionary
            Dim numeratorRow As DataRow = Nothing
            Dim denominatorRow As DataRow = Nothing
            Dim attribute = CStr(factorDicationaryEntry.Value("Attribute"))
            Dim differenceColumnName As String = String.Format("{0}Difference", attribute)

            ' try and get the rows that make up the F1 value
            numeratorSourceRowDictionary.TryGetValue(factorDicationaryEntry.Key, numeratorRow)
            denominatorSourceRowDictionary.TryGetValue(factorDicationaryEntry.Key, denominatorRow)

            Dim parseValue As Double
            Dim numeratorValue As Double? = Nothing
            Dim denominatorValue As Double? = Nothing

            ' attempt to get the numerator value if possible
            If (Not numeratorRow Is Nothing AndAlso Not numeratorRow(attributeValueColumnName) Is DBNull.Value) Then
                If (Double.TryParse(numeratorRow(attributeValueColumnName).ToString, parseValue)) Then
                    numeratorValue = parseValue
                End If
            End If

            ' attempt to get the denominator value if possible
            If (Not denominatorRow Is Nothing AndAlso Not denominatorRow(attributeValueColumnName) Is DBNull.Value) Then
                If (Double.TryParse(denominatorRow(attributeValueColumnName).ToString, parseValue)) Then
                    denominatorValue = parseValue
                End If
            End If

            ' if one of the values is missing
            If (numeratorValue Is Nothing Or denominatorValue Is Nothing) Then
                ' The factor result must be zero'd out...this is consistant with report behaviour
                factorDicationaryEntry.Value(attributeValueColumnName) = 0
                ' if there is an Difference column
                If (factorDicationaryEntry.Value.Table.Columns.Contains(differenceColumnName)) Then
                    factorDicationaryEntry.Value(differenceColumnName) = 0
                End If
            Else
                ' if there are values
                If (Math.Abs(denominatorValue.Value - 0) < Double.Epsilon) Then
                    ' if the denominator is zero then avoid divide-by-zero
                    factorDicationaryEntry.Value(attributeValueColumnName) = 0
                Else
                    ' otherwise calculate the new factor by dividing the numerator by the denominator
                    factorDicationaryEntry.Value(attributeValueColumnName) = numeratorValue.Value / denominatorValue.Value
                End If

                ' if there is an Difference column
                If (factorDicationaryEntry.Value.Table.Columns.Contains(differenceColumnName)) Then
                    ' then populate it based on the source rows
                    factorDicationaryEntry.Value(differenceColumnName) = numeratorValue.Value - denominatorValue.Value
                End If
            End If
        Next
    End Sub

    Private Shared Function GetF1F2F3ReportData(session As ReportSession,
         locationId As Int32, dateFrom As DateTime, dateTo As DateTime, Optional ByVal willAggregate As Boolean = True) As DataTable

        Dim holdingDataTable As DataTable = GetHubReportSet(session, locationId, dateFrom, dateTo).ToDataTable(session)
        ...
        For Each row As DataRow In holdingDataTable.Rows()
            ...
            If (tagIdForComparison.EndsWith(productSizeForRow) AndAlso Not productSizeForRow Is Nothing AndAlso productSizeForRow.Length > 0) Then
        ...
        If session.IncludeAsShippedModelsInHubSet Then
            Dim totalRows = holdingDataTable.AsEnumerable.WithProductSize("TOTAL")
            totalRows.Where(Function(r) r.AsString("ReportTagId").EndsWith("_AS")).DeleteRows()
            totalRows.Where(Function(r) r.AsString("ReportTagId").EndsWith("_AD")).DeleteRows()
            holdingDataTable.AsEnumerable.Where(Function(r) r.AsString("ReportTagId").StartsWith("F15")).SetFieldIfNull("RootCalcId", "F15Factor")
        End If
    ...
    Public Shared Sub FixF2Density(ByRef table As DataTable)
        Dim isPivotedTable As Boolean = Not table.Columns.Contains("AttributeValue")
        Dim attributesToCopy = New String() {"Density", "Volume"}

        For Each row As DataRow In table.AsEnumerable.Where(Function(r) r("ReportTagId").ToString.StartsWith("F2Density")).ToList()
            Dim tagId As String
            ...
            If tagId Is Nothing Then Continue For
        ...
        If (table.Columns.Contains(densityValueColumnName)) Then
            ...
            For Each row As DataRow In table.Rows
                ...
                If ((Not checkAttributeNameColumn) OrElse
                    ((Not row("Attribute") Is DBNull.Value) AndAlso (Not row("Attribute") Is Nothing) AndAlso row("Attribute").ToString() = "Density")) Then
                    ' and there is a value
                    If ((Not row(densityValueColumnName) Is DBNull.Value) AndAlso (Not row(densityValueColumnName) Is Nothing)) Then
    ...
    Private Shared Function GetFactorsForReportData(holdingDataTable As DataTable) As DataTable
        ...
        For i As Int32 = reportDataTable.Rows.Count - 1 To 0 Step -1
            ...
            If (tagId.EndsWith(productSize) AndAlso productSize.Length > 0) Then
    ...
    Public Shared Function GetAttributeValueFormat(attributeName As String, calcId As String) As String
        Dim isFactor = calcId.EndsWith("Factor") OrElse Calculation.RecoveryFactors.Contains(calcId)
    ...
    Public Shared Function GetThresholdValue(row As DataRow, thresholds As DataTable, attributeName As String) As String
        ...
        ' if this is a pivoted table, then the field with the value in it is the same as the
        ' attributeName
    ...
    Public Shared Function AddLocationDataToTable(ByRef session As ReportSession, ByRef table As DataTable, parentLocationId As Integer) As DataTable
        ...
        If parentLocationType = "BENCH" Then
            ...
            If pitLocation IsNot Nothing Then
                locationPrefix = String.Format("{0}-{1}-", pitLocation.AsString("Name"), parentLocation.LocationName)
            End If
    ...
    Public Shared Function GetTagOrderNo(calcId As String, Optional tagId As String = "") As Integer
        ...
        If calcId = "MiningModel" And tagId.StartsWith("F3MiningModel") Then
            orderNo += 1
        End If
    ...
    ''' </summary>
    Public Shared Function GetAllLocationNamesRecursive(session As ReportSession, parentLocation As Int32, dateTime As Date, Optional lowestLocationType As String = "Pit") As Dictionary(Of Int32, ExtendedLocation)
    ...
    ''' Retrieves a dictionary of all the location ids, location definitons of the supplied parent id and ALL it's children.
    ''' </summary>
    Public Shared Function GetAllLocationNames(session As ReportSession, parentLocation As Int32) As Dictionary(Of Int32, Types.Location)
        ...
        ' Get parent location
        Dim locationNames As Dictionary(Of Integer, Types.Location) = GetParentLocationName(session, parentLocation)

        ' Get child locations
        Dim locationTable As DataTable = session.DalUtility.GetBhpbioLocationListWithOverride(parentLocation, 1, locationDate)
    ...
    Private Shared Function GetParentLocationNameWithOverride(session As ReportSession, parentLocationId As Int32, startDate As Date, endDate As Date) As Dictionary(Of Int32, Types.Location)
        Dim locationTable As DataTable
    ...
    Private Shared Function GetParentLocationName(session As ReportSession, parentLocationId As Int32) As Dictionary(Of Int32, Types.Location)
        Dim locationTable As DataTable
...
#End Region
    ...
    Public Shared Function GetFactorsForProductType(session As ReportSession, dateFrom As DateTime,
        ...
        Dim productTypeLocationInfo As Types.Location = locationInformation(session.SelectedProductType.LocationId)

        Dim productTypeResultTable As DataTable = GetF1F2F3ReportData(session, session.SelectedProductType.LocationId, dateFrom, dateTo, Not groupOnCalendarDate)
    ...
    Public Shared Function GetFactorsForProductTypes(session As ReportSession, dateFrom As DateTime,
        ...
        Dim productTypeIdStrings As String() = productTypeIdsString.Split({","c})
    ...
    Public Shared Function GetFactorsForProductTypes(session As ReportSession, dateFrom As DateTime,
                                                          dateTo As DateTime,
                                                          productTypeIds As IEnumerable(Of Integer),
                                                          groupOnCalendarDate As Boolean) As DataTable
        Dim resultsDataTable As DataTable = Nothing

        If Not productTypeIds Is Nothing Then
            For Each productTypeId As Integer In productTypeIds
    ...
    Public Shared Function GetFactorsForLocations(session As ReportSession, dateFrom As DateTime,
        ...
        For Each location In locations
            ...
            Else
                Dim holdingTable As DataTable = GetF1F2F3ReportData(session, location.Key, dateFrom, dateTo)
