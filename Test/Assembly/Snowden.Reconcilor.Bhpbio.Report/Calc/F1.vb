...
Namespace Calc
    Public NotInheritable Class F1
        Inherits Calculation
        Implements IAllMaterialTypesCalculation

        Public Const CalculationId As String = "F1Factor"
        Public Const CalculationDescription As String = "F1 - Grade Control Model / Mining Model"
        ...
        Protected Overrides Sub SetupOperation()
            ...
            Dim gradeControlModelCalc As Calc.IAllMaterialTypesCalculation = _
                DirectCast(Calculation.Create(CalcType.ModelGradeControl, Session), Calc.IAllMaterialTypesCalculation)
            Dim miningModelCalc As Calc.IAllMaterialTypesCalculation = _
                DirectCast(Calculation.Create(CalcType.ModelMining, Session), Calc.IAllMaterialTypesCalculation)

            gradeControlModelCalc.IncludeAllMaterialTypes = Me.IncludeAllMaterialTypes
            miningModelCalc.IncludeAllMaterialTypes = Me.IncludeAllMaterialTypes

            Dim gradeControlResult As CalculationResult = DirectCast(gradeControlModelCalc, Calc.ICalculation).Calculate()
            Dim miningModelResult As CalculationResult = DirectCast(miningModelCalc, Calc.ICalculation).Calculate()

            gradeControlResult.PrefixTagId("F1")
            miningModelResult.PrefixTagId("F1")
        ...
        Protected Overrides Sub ProcessTags()
            ...
            Dim gradeControlCalc As Calc.CalculationModel = DirectCast(Calculation.Create(CalcType.ModelGradeControl, Session), Calc.CalculationModel)
            ...
            gradeControlCalc.IncludeAllMaterialTypes = Me.IncludeAllMaterialTypes
            miningModelCalc.IncludeAllMaterialTypes = Me.IncludeAllMaterialTypes
            ...
            difference = CalculationResult.PerformCalculation(gradeControlResult, modelResult, CalculationType.Difference)
