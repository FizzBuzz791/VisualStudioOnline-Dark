...
Namespace Reports
    Public Class GetBlastblockDataExportReport
        ...
        Private Sub GenerateReport()
            ...
            Me.SaveUserSettings(locationId, dateFrom, dateTo, approvalSelection)

            Dim service As New Snowden.Reconcilor.Bhpbio.Report.WebService
            service.SetOverrideReportContext(reportApprovalContext)

            Dim data As DataTable = service.GetBlastblockDataExportReport(locationId, dateFrom, dateTo)
            ...
            Using package As New ExcelPackage()
                ...
                paramsWorkSheet.Cells("A1").LoadFromDataTable(GetReconciliationDataExportReport.CreateParametersTable(locationName,
                    dateFrom, dateTo, approvalSelection, Nothing, Nothing, Nothing, Nothing), True)
                ...
                Response.AppendHeader("Content-Disposition", String.Format("attachment; filename={0}", "Blastblock_Data_Export.xlsx"))
                Response.BinaryWrite(package.GetAsByteArray())
            End Using
        End Sub

        Private Sub SaveUserSettings(ByVal locationId As Integer, ByVal dateFrom As DateTime, ByVal dateTo As DateTime, ByVal approvalStatus As String)

            Resources.UserSecurity.SetSetting("Blastblock_Data_Export_Location_Id", locationId.ToString)
            Resources.UserSecurity.SetSetting("Blastblock_Data_Export_Date_From", dateFrom.ToString(Application("DateFormat").ToString))
            Resources.UserSecurity.SetSetting("Blastblock_Data_Export_Date_To", dateTo.ToString(Application("DateFormat").ToString))
            Resources.UserSecurity.SetSetting("Blastblock_Data_Export_Approval_Status", approvalStatus.ToString)
        End Sub
