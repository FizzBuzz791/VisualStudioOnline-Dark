...
'MMM/dd/yyyy');
    ...
    function validateDate(eInput, bRequired, dStartDate, dEndDate) {
        ...
        if (formatMatchCount > 1) {
            ...
            //check that the date doesn't violate SqlDateTime limits (i.e. minimum of 01-01-1753)
            var passedDate = getDateFromFormat(eInput.value, this.AllowedFormats[firstMatchAt]);
            var minDate = new Date(1900, 0, 3, 0, 0, 0, 0)
            if (passedDate < minDate) {
                return false;
            }
        }
        ...
        if (bOK == true) {
            ...
            if (dStartDate != null && dStartDate != '') {
                //Required d-MMM-yyyy	
                var dStart = getDateFromFormat(dStartDate, "d-MMM-yyyy");
            ...
            if (dEndDate != null && dEndDate != '') {
                //Required d-MMM-yyyy	
                var dEnd = getDateFromFormat(dEndDate, "d-MMM-yyyy");
                var dThis = getDateFromFormat(eInput.value, this.AllowedFormats[firstMatchAt]);
                if (dEnd < dThis) {
                    eInput.className = "cal-TextBoxInvalid";
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
    ...
    */
    // Generic Properties
    this.varName = 'CalendarControls.Lookup(\'' + strVarName + '\')';
    this.enabled = true;
    this.readonly = false;
    this.visible = false;
    ...
    switch (intBtnMode) {
        ...
        case 3:
            ...
            img_DateBtn_UP.src = strDefBtnImgPath + 'calendar.gif';
            img_DateBtn_OVER.src = strDefBtnImgPath + 'calendar.gif';
            img_DateBtn_DOWN.src = strDefBtnImgPath + 'calendar.gif';
            img_DateBtn_DISABLED.src = strDefBtnImgPath + 'calendar.gif';
            strBtnW = '16';
            strBtnH = '15';
            break;
    }
    ...
    this.maxYearChoice = dteToday.getFullYear() + 21;
    ...
    function GetYearRange() {
        ...
        var max;
        $.ajax({
            url: "../Utilities/GetCulture.aspx?a=" + Math.random(), //appending random number onto query string to force IE to fire AJAX request to server instead of using cached result of previous call
            type: 'GET',
            contentType: "application/json; charset=utf-8",
            dataType: 'json',
            async: false,
    ...
    if ( minY != null && maxY != null && parseInt(minY) > 0 && parseInt(maxY) > 0 && (parseInt(minY) < parseInt(maxY)) ) {
        this.minYearChoice = minY;
        this.maxYearChoice = parseInt(maxY) + 1;//add 1 because in the for loop that creates drop-down options one year is cut off (i.e. it uses < condition instead of <= condition)
    }
    ...
    */
    ...
    function makeCalendar(intWhatMonth, intWhatYear, bViewOnly) {
        ...
        if (scNN6) {
            strOutput += strTemp + '<table width="185" border="3" style="z-index:99999" class="cal-Table" cellspacing="0" cellpadding="0"><tr>';
        }
        else {

            strOutput += '<table width="185" border="3" style="z-index:99999;" class="cal-Table" cellspacing="0" cellpadding="0">' + strTemp + '<tr>';
        }
    ...
    function getControlHTML() {
        ...
        // Alert the developer if the form name is not set, this is a requirement.
        // This is to help developers get over the difficulties surrounding spiffy cal.
        // Please delete this control (Murray)
        if (this.formName == '') {
            alert('Calender control ' + this.textBoxName + ' does not have it\'s form set.');
            return '';
        }
        ...
        strHold += '<img align="absmiddle" border="0" name="' + this.btnName + '" src="' + strBtnImg + '" width="' + strBtnW + '" height="' + strBtnH + '" hspace=5></a>';
    ...
    function changeDay(intWhatDay) {
        ...
        calMgr.validateDate(this.textBox(), this.required);
    ...
    function isDayValid(intWhatDay, intWhatMonth, intWhatYear) {
        ...
        dteCur.setFullYear(intWhatYear);
        dteCur.setHours(0); dteCur.setMinutes(0); dteCur.setSeconds(0); dteCur.setMilliseconds(0);
        if ((this.minDate == null || dteCur >= this.minDate) && (this.maxDate == null || dteCur <= this.maxDate)) {
            return true;
        }
        else {
            return false;
...
function domlay(id, trigger, lax, lay, content) {
    ...
    */
    ...
    // Hack to ensure that if spiffy calender has not got an Iframe, it will make one.
    // This has caught out too many people to be ignored, This control needs to go. (Murray)
    if (!document.all['' + id + '']) {
        var divElement = document.createElement('div');
        divElement.id = id;
        divElement.style.zIndex = "101";
        document.body.appendChild(divElement);
        // Otherwise, could just alert developer with the following message.
        //alert('Could not find div to show. \nHas the Me.HasCalendarControl = True been set before MyBase.SetupPageLayout()?');
    }
    if (!document.all['' + id + 'iFrameShield']) {
        var frameElement = document.createElement('iframe');
        frameElement.id = id + 'iFrameShield';
        frameElement.src = '';
        frameElement.style.position = "absolute";
        frameElement.style.zIndex = "100";
        frameElement.style.visibility = "hidden";
        frameElement.width = '190px';
        frameElement.height = '105px';
    ...
    if (trigger == "1") {
        if (document.layers) document.layers['' + id + ''].visibility = "show"
            ...
            document.all['' + id + 'iFrameShield'].style.visibility = "visible"
        }
    ...
    // Layer hidden
        ...
        if (document.layers) document.layers['' + id + ''].visibility = "hide"
            ...
            document.all['' + id + 'iFrameShield'].style.visibility = "hidden"
        }
    ...
    if (lax) {
        if (document.layers) { document.layers['' + id + ''].left = lax }
            ...
            document.all['' + id + 'iFrameShield'].style.left = lax
        }
    ...
    if (lay) {
        if (document.layers) { document.layers['' + id + ''].top = lay }
            ...
            document.all['' + id + 'iFrameShield'].style.top = lay
        }
